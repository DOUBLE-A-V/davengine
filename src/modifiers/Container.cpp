...
void Container::DrawInterface() {
  ...
  for (Object *child : parentObject->children) {
    ...
    if (!hasConstraintor) {
      child->props->scale =
          Vector2{child->props->localScale.x * props->scale.x,
                  child->props->localScale.y * props->scale.y};

      child->props->rotation = props->rotation + child->props->localRotation;
      child->props->position.x =
          props->position.x + (child->props->localPosition.x * sinRot +
                               child->props->localPosition.y * cosRot) *
                                  props->scale.x;

      child->props->position.y =
          props->position.y + (-child->props->localPosition.y * sinRot +
